...
//
// $Id: BmpUtil.h 5696 2012-03-02 22:30:04Z shun $
//
...
namespace
{
inline float rgb_to_intensity(float r, float g, float b)
{
    ...
    unsigned char *const dst)
{
	...
	for (int y = 0; y < h; y++)
	{
		for (int x = 0; x < w / 2; x++)
		{
			...
			for (int c = 0; c < ch; c++)
			{
				std::swap(dst[y * line + ch * x + c], dst[y * line + ch * r + c]);
			}
		}
	}
}
    ...
    unsigned char *const dst)
{
	...
	for (int i = 0; i < h / 2; i++)
	{
		memcpy(tmp, dst + line * i, line);
		memcpy(dst + line * i, dst + line * (h - i - 1), line);
...
//			printf("%d, %d\n", x, y);
	...
	void Initialize(int x, int y, int ch)
	{
		m_width = x;
		m_height = y;
		m_num_channels = ch;
	...
	void ChangeNumChannels(int n)
	{
		int org = GetNumChannels();
		if (n == org)
		{
			return;
		}
		...
		case 1:
			...
			{
			case 3:
				p = new unsigned char [wh * 3];
				Gray2Rgb(wh, m_pixel, p);
				delete [] m_pixel;
				m_pixel = p;
				break;
			case 4:
				p = new unsigned char [wh * 4];
				Gray2Rgba(wh, m_pixel, p);
				delete [] m_pixel;
				m_pixel = p;
				break;
			}
		...
		case 3:
			...
			{
			case 1:
				Rgb2Gray(wh, m_pixel, m_pixel);
				break;
			case 4:
				p = new unsigned char [wh * 4];
				Rgb2Rgba(wh, m_pixel, p);
				delete [] m_pixel;
				m_pixel = p;
				break;
			}
